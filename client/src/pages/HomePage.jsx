...
const HomePage = () => {
  ...
  return (
    <div className="h-screen overflow-y-scroll flex flex-col text-white font-sans bg-[url('/bg-img.png')] 
    bg-cover bg-center bg-no-repeat">
      ...
      <nav className="sticky top-0 z-10 flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className='text-xl font-semibold tracking-tight'>Weave AI</span>
        </div>
        <div className='flex items-center gap-4 text-sm font-medium text-zinc-300'>
          <span>{user?.name}</span>
          <button onClick={logout} className='py-1.5 px-3 border border-white/20 
          text-white hover:bg-white/10 text-xs rounded-md cursor-pointer bg-transparent'>
            Sign out
          </button>
        </div>
      </nav>
      ...
      <div className='flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28'>
        <div className='w-full max-w-2xl flex flex-col items-center text-center'>
          {/**Promo Badge */}
          <div className="flex items-center gap-2 p-1.5 pr-3 bg-white/10 backdrop-blur-md rounded-full 
          border border-white/20 text-[13px] text-white/90">
          ...
          {/**Title */}
          <h1 className='text-center text-4xl md:text-6xl font-medium mt-6 max-w-2xl text-white tracking-tight leading-tight'>
          ...
          {!loadingProjects && projects.length > 0 && (
            <div className='mt-10 w-full'>
              ...
              <div className='flex items-center justify-between mb-3 text-xs'>
                <p className='text-[11px] font-semibold tracking-wider uppercase text-white/80'>ALL PROJECTS</p>
              ...
              <div className='space-y-3 max-h-[80vh] overflow-y-auto pr-1'>
                {projects.map((p)=>(
                  <div
                    key={p._id}
                    className='bg-white/5 border border-white/10 rounded-xl px-5 py-3.5 flex items-center justify-between group hover:border-white/20 hover:bg-white/10 cursor-pointer backdrop-blur-md transition-all'
                    ...
                    <div className='flex-1 min-w-0 text-left'>
                      ...
                      <div className='flex items-center gap-3 mt-1'>
                        <span className='text-xs text-white/70 flex items-center gap-1.5'>
                          ...
                          {DateTime.fromISO(p.updatedAt || p.createdAt).toRelative()}
                        </span>
