...
const PromptInput = ({onSubmit, loading = false, placeholder = "Describe the website you want to BiquadFilterNode...", large = false, autoFocus = false, variant = "default"}) => {
    ...
    if(variant == "glass"){
        ...
        return (
            <form onSubmit={handleSubmit} className='max-w-2xl w-full bg-white/10 backdrop-blur-xl rounded-xl ring-1 ring-white/25
            focus-within:ring-2 focus-within:ring-white/30 overflow-hidden mt-6 transition'>
                ...
                <div className='flex items-center justify-between pb-3 px-3 gap-2'>
                    <label htmlFor='file' className='border border-white/20 text-white/80 
                    hover:text-white hover:border-white/30 p-1.5 rounded-md cursor-pointer flex 
                    ...
                    <div className='flex items-center justify-end gap-2'>
                        ...
                        <button type='submit' disabled={!value.trim() || loading} className='flex items-center justify-center p-1.5 rounded-full bg-red-600 text-white
    ...
    focus-within:ring-1 focus-within:ring-zinc-300 transition ${large ? "p-4" : "p-3"}`}>
        ...
        <textarea ref={textareaRef} 
            ...
            disabled={loading}
            rows={large ? 5 : 1} 
            className={`flex-1 bg-transparent border-none outline-none resize-none text-zinc-900
                placeholder:text-zinc-400 ${large ? "text-base" : "text-sm"}`}
        />
